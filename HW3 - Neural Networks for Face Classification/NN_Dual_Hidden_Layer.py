...
# 0310128 Daniel You
# Homework 3 - Face Classification (Neural Network - Dual Hidden Layer)
...
#l_rate = [0.0001, 0.0001, 0.0001] #[0.001, 0.001, 0.001] 
l_rate = [0.10, 0.10, 0.10]
n_epoch = 200
scaling = 10.0 #0.001
...
NN_Layer = [reduce_dimension, hidden_1, hidden_2, numClass]
...
for itr_cv in range(cv_count):
	...
	#classify_result = nn.nn_prediction(network, train_normal, bReLU)
	classify_result = nn.nn_prediction(network, test_normal, bReLU)
	#err_itr = nn.determineCorrectness(classify_result, train_class, trainAmt, \
...
nn.exportData3d("gen_2H.csv", gen[:, 0] \
							, gen[:, 1] \
							, gen_result)
