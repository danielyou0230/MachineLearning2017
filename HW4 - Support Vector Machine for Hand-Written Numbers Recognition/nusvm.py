...
testing_data, testing_targ   = load_data(file_testing_data, \
										 file_testing_targ)
# 
svm_mode = ['poly', 'rbf'] #['linear', 'poly', 'rbf']
poly_order = [2, 3, 4]
nu_resolution = 100.0
...
for itr_mode in svm_mode:
	...
	for itr_order in orderLst:
		for itr_nu in range(1, 101):
		#for itr_nu in range(1, int(nu_resolution + 1)):
			
			###
			v = itr_nu / nu_resolution
			model = NuSVC(nu=v, kernel=itr_mode, degree=itr_order, gamma=1/392.0)
			...
			if itr_mode == 'poly':
				print "Order = {:d}, Nu = {:1.2f}, error rate: {:.2f}% ({:4d}/{:4d})" \
					  .format(itr_order, v, \
